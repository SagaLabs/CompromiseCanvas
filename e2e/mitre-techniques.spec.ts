...
test('searches the real catalog by ID, name, and tactic and normalizes legacy data', () => {
  ...
  expect(
    searchMitreTechniques('unsecured credentials').some((technique) => technique.id === 'T1552'),
  ).toBeTruthy();
  expect(
    searchMitreTechniques('credential-access').some((technique) => technique.id === 'T1552'),
  ).toBeTruthy();
...
test('adds, removes, links, and persists multiple techniques from legacy edge data', async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole('complementary').getByRole('link', { name: /Open T1552/ }),
  ).toHaveAttribute('href', 'https://attack.mitre.org/techniques/T1552');
  await page
    .getByRole('complementary')
    .getByRole('combobox')
    .filter({ hasText: 'Add technique' })
    .click();
  await page.getByPlaceholder('Search techniques…').fill('T1059');
  await page
    .getByRole('option')
    .filter({ hasText: 'T1059' })
    .filter({ hasText: 'Command and Scripting Interpreter' })
    .first()
    .click();
  await expect(
    page.getByRole('button', { name: 'T1059 — Command and Scripting Interpreter', exact: true }),
  ).toBeVisible();
  ...
  expect(savedTechniques.map((technique: { id: string }) => technique.id)).toEqual([
    'T1552',
    'T1059',
  ]);
...
test('renders an invalid legacy technique as plain text without a broken link', async ({
  page,
}) => {
  ...
  const edgeLabel = page
    ...
    .filter({ hasText: 'LEGACY-ID — Legacy technique' });
