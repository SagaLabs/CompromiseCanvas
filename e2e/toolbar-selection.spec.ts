...
test('node toolbar compromised toggle stays synchronized and is one undo step', async ({
  page,
}) => {
...
test('investigation quick action stays synchronized and is one undo step', async ({ page }) => {
  ...
  const investigationSelect = page
    .getByText('Investigation Status', { exact: true })
    .locator('..')
    .getByRole('combobox');
...
test('copy after a toolbar edit uses the live node data', async ({ page }) => {
  ...
  const compromisedStates = await page.evaluate(() => {
    ...
    return saved.nodes.map((node: { data: { isCompromised?: boolean } }) =>
