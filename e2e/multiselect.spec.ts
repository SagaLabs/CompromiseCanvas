...
function nodeDataById(graph: { nodes: Array<{ id: string; data: Record<string, unknown> }> }) {
  return new Map<string, Record<string, unknown>>(graph.nodes.map((item) => [item.id, item.data]));
}
...
test('Shift-click toggles selection and Shift-drag creates a full-containment marquee', async ({
  page,
}) => {
  ...
  const [aBox, bBox] = await Promise.all([
    node(page, 'a').boundingBox(),
...
test('held-Shift clicks tolerate pointer jitter and copy every selected asset', async ({
  page,
}) => {
  await seedDiagram(page);
  ...
  expect(
    pasted.nodes.filter((item: { id: string }) => item.id.startsWith('copied_node_')),
  ).toHaveLength(2);
});
...
test('copy/paste preserves parallel internal edges, excludes external edges, and undoes atomically', async ({
  page,
}) => {
  const snapshot = {
    ...baseSeed,
  ...
  const pasted = await saveGraph(page);
  const copiedNodes = pasted.nodes.filter((item: { id: string }) =>
    item.id.startsWith('copied_node_'),
  );
  const copiedNodeIds = new Set(copiedNodes.map((item: { id: string }) => item.id));
  const copiedEdges = pasted.edges.filter((item: { id: string }) =>
    item.id.startsWith('copied_edge_'),
  );
...
test('selection alignment and distribution affect only selected nodes and each undo atomically', async ({
  page,
}) => {
  await seedDiagram(page);
  ...
  const firstGap =
    positions.get('b')!.x - width('b') / 2 - (positions.get('a')!.x + width('a') / 2);
  const secondGap =
    positions.get('c')!.x - width('c') / 2 - (positions.get('b')!.x + width('b') / 2);
...
test('bulk compromised and investigation status changes are atomic and selection-scoped', async ({
  page,
}) => {
  const snapshot = {
    ...baseSeed,
    nodes: baseSeed.nodes.map((item) =>
      item.id === 'c' ? { ...item, data: { ...item.data, type: 'attacker' } } : item,
    ),
  };
  await seedDiagram(page, snapshot);
  await selectNodes(page, ['a', 'b', 'c']);
...
test('right-clicking a selected asset preserves bulk actions without triggering pane paste', async ({
  page,
}) => {
  await seedDiagram(page);
  await selectNodes(page, ['a', 'b']);
