...
test('unlock toggle enables rerouting; drag bends the edge and moves its label', async ({
  page,
}) => {
  await seedDiagram(page);
  const path = edgePath(page);
...
test('Shift-clicking a locked edge label keeps the route selected and highlighted', async ({
  page,
}) => {
  await seedDiagram(page);
...
test('toolbar edge changes remain intact after a properties edit', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Lock edge' })).toBeVisible();
  await expect(
    page.locator('.react-flow__edgelabel-renderer').getByText('Impact', { exact: true }),
  ).toBeVisible();
