...
const spreadNodesOnAxis = (nodes, axis, nextPositions, gap = 32) => {
  ...
  const span =
    sorted.reduce((total, node) => total + nodeSize(node, axis), 0) + gap * (sorted.length - 1);
...
export const layoutSelectedNodes = (nodes, action) => {
  ...
  if (action === 'distribute-horizontal') {
    ...
    const innerWidth = sorted
      .slice(1, -1)
      .reduce((total, node) => total + nodeSize(node, 'width'), 0);
    const available =
      last.position.x -
      nodeSize(last, 'width') / 2 -
  ...
  if (action === 'distribute-vertical') {
    ...
    const innerHeight = sorted
      .slice(1, -1)
      .reduce((total, node) => total + nodeSize(node, 'height'), 0);
    const available =
      last.position.y -
      nodeSize(last, 'height') / 2 -
