...
export const useCompromiseCanvasState = () => {
  ...
  const allBulkStatusNodesCompromised =
    bulkStatusNodes.length > 0 && bulkStatusNodes.every((node) => node.data.isCompromised);
  const bulkInvestigationStatus =
    bulkStatusNodes.length > 0 &&
    bulkStatusNodes.every(
      (node) => node.data.investigationStatus === bulkStatusNodes[0].data.investigationStatus,
    )
      ? bulkStatusNodes[0].data.investigationStatus
      : null;
      ...
      if (storedSnapshot) {
        try {
          ...
          const restoredTimestamp =
            snapshot.timestamp || localStorage.getItem(AUTOSAVE_TIMESTAMP_KEY);
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      if (typeof window.requestIdleCallback === 'function') {
        idleCallbackId = window.requestIdleCallback(writeAutosave, {
  ...
  const handleUndo = useCallback(() => {
    ...
    if (previousState) {
      ...
      setNodes(
        previousState.nodes.map((node) => ({ ...node, selected: selectedNodeIds.has(node.id) })),
      );
      setEdges(
        previousState.edges.map((edge) => ({ ...edge, selected: selectedEdgeIds.has(edge.id) })),
      );
    }
  ...
  const handleRedo = useCallback(() => {
    ...
    if (nextState) {
      ...
      const selectedEdgeIds = new Set(edges.filter((edge) => edge.selected).map((edge) => edge.id));
      setNodes(
        nextState.nodes.map((node) => ({ ...node, selected: selectedNodeIds.has(node.id) })),
      );
      setEdges(
        nextState.edges.map((edge) => ({ ...edge, selected: selectedEdgeIds.has(edge.id) })),
      );
    }
