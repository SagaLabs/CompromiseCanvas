...
function getSelfLoopGeometry({
  ...
  const nodeClearanceHeight = ((nodeHeight / 2 + SELF_LOOP_LABEL_CLEARANCE_PX) * 4) / 3;
...
const CustomEdge = memo(function CustomEdge({
  ...
  const sourceNodeHeight = useStore((state) => state.nodeLookup.get(source)?.measured.height ?? 0);
  ...
  const pulseAnimation =
    selected && !multiSelectionActive ? 'edge-pulse 1.5s ease-in-out infinite' : '';
  ...
  return (
    <>
      <BaseEdge id={id} path={edgePath} style={{ ...style, ...edgeStyle }} />
      ...
      {data?.displaySettings?.showLabel !== false && (
        <EdgeLabelRenderer>
          ...
          >
            ...
            <div
              className="mb-1 flex items-center justify-center gap-2 text-sm font-semibold"
              style={{ color: edgeStyle.stroke }}
            >
            ...
            <div className="space-y-1 text-gray-400">
              ...
              {data?.actionType === 'Command & Control' &&
                data?.c2Channel &&
