...
const useDownloadImage = () => {
  ...
  const [, setBackgroundType] = useState<'dark' | 'transparent'>('dark');
...
export default function DownloadButton(_props: DownloadButtonProps) {
  const { onDownloadClick } = useDownloadImage();
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          className="z-10 rounded border-blue-600 bg-blue-600 px-4 py-2 text-white shadow-lg transition-colors hover:bg-blue-700"
      ...
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuItem onClick={() => onDownloadClick('dark')}>
          <div className="flex items-center">
            <div className="mr-2 h-4 w-4 rounded bg-gray-800"></div>
            Dark Background
          </div>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => onDownloadClick('transparent')}>
