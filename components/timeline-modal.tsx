...
import { Activity, ArrowRight, Clock, Filter, Search, StickyNote, X } from 'lucide-react';
...
export default function TimelineModal({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="bottom" className="ip-panel ip-text ip-border border-t p-0">
        <SheetHeader className="ip-panel-muted ip-border border-b px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="ip-border rounded-lg border p-2">
                <Activity className="ip-accent h-5 w-5" aria-hidden="true" />
              </div>
              <div>
            ...
            <div className="flex flex-1 items-center gap-3 sm:justify-end">
              <div className="relative w-full max-w-sm">
                <Search
                  className="ip-text-muted pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2"
                  aria-hidden="true"
                />
                <Input
                  value={query}
              ...
              <div className="ip-border flex items-center gap-2 rounded-full border px-3 py-1 text-xs">
                <Switch
                  checked={includeIncidentLog}
                  onCheckedChange={setIncludeIncidentLog}
                  aria-label="Include incident log"
                />
          ...
          {availableActionTypes.length > 0 && (
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <div className="ip-text-muted flex items-center gap-2 text-xs">
              ...
              {availableActionTypes.map((actionType) => {
                ...
                const tone =
                  actionTypeTone[actionType] || 'text-gray-300 border-gray-400/40 bg-gray-500/10';
        ...
        <ScrollArea className="h-[60vh] px-6 py-4">
          {!hasEvents ? (
            <div className="ip-text-muted flex flex-col items-center justify-center gap-3 py-16">
              ...
              {filteredEvents.map((event, index) => {
                ...
                const tone =
                  event.kind === 'edge' && event.actionType
                    ? actionTypeTone[event.actionType] ||
                ...
                return (
                  <div
                    key={event.id}
                    className="ip-panel-muted ip-border rounded-lg border px-4 py-3 transition hover:bg-gray-700"
                    onClick={() => {
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        {isIncident && event.incidentCategory && (
                          <span className="ip-text-muted rounded-full border px-2 py-0.5 text-xs">
                            {event.incidentCategory}
                          </span>
                        )}
                        <span className="ip-text-muted text-xs">
                          {formatIsoSeconds(event.parsedDate)}
                        </span>
                      </div>
                      <div className="ip-text-muted flex items-center gap-3 text-xs">
                    ...
                    {isIncident ? (
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                        <StickyNote className="ip-text-muted h-4 w-4" />
                        <span className="ip-text font-medium">Incident Log Entry</span>
                      </div>
                    ) : (
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                        <span className="ip-text truncate font-medium">{sourceLabel}</span>
                        <ArrowRight className="ip-text-muted h-4 w-4" />
                        <span className="ip-text truncate font-medium">{targetLabel}</span>
                      </div>
                    )}
                      ...
                      <>
                        {!isIncident &&
                          (event.toolUsed ||
                            ...
                            event.mitreAttackId ||
                            event.description) && (
                            <div className="ip-text-muted mt-2 grid gap-1 text-xs">
                              {event.toolUsed && <div>Tool: {event.toolUsed}</div>}
                              {event.userUsed && <div>User: {event.userUsed}</div>}
                              {event.mitreAttackTechniques &&
                                event.mitreAttackTechniques.length > 0 && (
                              ...
                              {event.description && (
                                <div className="ip-text">{event.description}</div>
                              )}
                            </div>
                          )}
                        {event.description && isIncident && (
                          <div className="ip-text-muted mt-2 text-xs">{event.description}</div>
                        )}
                        {!isIncident && (
